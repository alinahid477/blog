#!/usr/bin/env bash
# poll-remediate-loop.sh
#
# Calls poll-remediate-services.py up to MAX_POLLS times (POLL_INTERVAL seconds
...
#
# Usage:
#   bash poll-remediate-loop.sh <job-id>
#
# Output (stdout, machine-parseable — always on their own lines):
#   STATUS=<status>               relayed from poll-remediate-services.py
#   REPORT_PATH=<path>            relayed when STATUS=successful or STATUS=partial
#   LOOP_RESULT=successful        terminal — job done, proceed
#   LOOP_RESULT=partial           terminal — partial results (unreachable hosts), proceed
#   LOOP_RESULT=still_running     non-terminal after MAX_POLLS — call again
#   LOOP_RESULT=<failed|error|canceled>   terminal — job failed
#   LOOP_RESULT=task_failure      terminal — AAP reported task-level failures
...
#   10 — job successful or partial; REPORT_PATH is in stdout
#   1  — job ended with a failure status or task failures
...
for poll in $(seq 1 $MAX_POLLS); do
    echo "  [Poll ${poll}/${MAX_POLLS}] Checking remediation job ${JOB_ID}..."

    # Capture stdout for parsing; stderr flows directly to the terminal
    OUTPUT=$(cd "$SCRIPTS_DIR" && python3 poll-remediate-services.py --job-id "$JOB_ID")
    EXIT_CODE=$?

    # Relay the Python script's stdout so the agent can see STATUS / REPORT_PATH
    echo "$OUTPUT"

    # ── Handle task failures (exit 3 from poll script) ────────────────────────
    if [ "$EXIT_CODE" -eq 3 ]; then
        echo "LOOP_RESULT=task_failure"
        exit 1
    fi
